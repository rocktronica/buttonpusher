...
def run(sequence, count=0):
    ...
    for item_index in range(0, count, 1):
        ...
        for (step_index, _) in enumerate(sequence):
            display.start_step(step_index)

            halt = wait.interruptible_sleep(
                get_seconds_to_wait(
                    item_index, step_index, sequence, get_seconds_elapsed()
                ),
                update_display,
            )
            if halt:
